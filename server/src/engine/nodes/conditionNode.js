 ...
 *
 * MOVED (MVP pass): This function now lives in ./nodeOutputUtils.js so
 * aiNode.js and notificationNode.js can share the exact same
 * implementation instead of each maintaining their own copy. Imported
 * below rather than redefined here.
 */
...
/**
 * evaluateContainsNegative
 *
 * WHY THIS EXISTS (MVP addition): The demo workflow needs to route a
 * negative-sentiment input down a DIFFERENT branch than a positive one
 * (per the "Customer Feedback Automation" example). Rather than hard-
 * coding the whole engine around one keyword, this is a second,
 * equally-configurable condition type a workflow author can pick from
 * the same dropdown as "contains_positive" — the architecture already
 * supported adding this as a one-function change (see the file header).
 */
function evaluateContainsNegative(text) {
  return text.toLowerCase().includes("negative");
}
...
async function executeConditionNode(node, previousOutput) {
  ...
  switch (conditionType) {
    ...
    case "contains_negative":
      result = evaluateContainsNegative(textToCheck);
      break;
    default:

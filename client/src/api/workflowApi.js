...
/**
 * Run a saved workflow against a piece of user-provided input.
 * Backend route: POST /api/workflows/:id/execute (JWT required)
 *
 * WHY this is separate from saveWorkflow():
 * Saving persists the graph definition; executing actually WALKS the
 * graph (via the backend engine) against real input and returns a
 * result — a fundamentally different action hitting a different route.
 *
 * @param {string} id - MongoDB _id of the workflow to run
 * @param {string} input - raw text typed into the "Run Workflow" box
 * @returns {Promise<Object>} backend response data:
 *   { execution, notification, stepResults, status }
 */
export const executeWorkflow = async (id, input) => {
  try {
    ...
    const message =
      error.response?.data?.message || "Failed to run workflow. Please try again.";
    throw new Error(message, { cause: error });
  }
};

/**
 * Fetch this user's full execution history, most recent first.
 * Backend route: GET /api/executions (JWT required)
 *
 * @returns {Promise<Object>} backend response data
 */
export const getExecutionHistory = async () => {
  try {
    const response = await apiClient.get("/executions", authHeaders());
    return response.data;
  } catch (error) {
    const message =
